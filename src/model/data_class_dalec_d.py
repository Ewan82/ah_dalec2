...
class DalecData:
    ...
    def __init__(self, start_date=None, end_date=None, ob_str=None,
        ...
        self.p1 = 2.5e-4  # theta_min, cl to cs decomp        (1e-6 - 1e-2) day-1
        self.p2 = 0.45  # f_auto, fraction of GPP respired    (0.2 - 0.7)
        self.p3 = 0.08  # f_fol, frac GPP to foliage          (0.01 - 0.5)
        self.p4 = 0.2  # f_roo, frac GPP to fine roots        (0.01 - 0.5)
        self.p5 = 1.2e-2  # turnover rate of foliage          (1e-4 - 0.1)  day-1
        self.p6 = 4.8e-5  # theta_woo, wood C turnover        (1e-6 - 1e-2) day-1
        self.p7 = 6.72e-3  # theta_roo, root C turnover rate  (1e-4 - 1e-2) day-1
        self.p8 = 0.024  # theta_lit, litter C turnover       (1e-5 - 1e-2) day-1
        self.p9 = 2.4e-5  # theta_som, SOM C turnover         (1e-6 - 1e-2) day-1
        self.p10 = 0.0193  # Theta, temp dependence exp fact  (0.018 - 0.2)
        self.p11 = 90.  # ceff, canopy efficiency param       (10 - 100)
        self.p12 = 240.  # GDD value causing leaf out         (200 - 400) degC
        self.p13 = 10.  # Min daily temp causing leaf fall    (8 - 15) degC
        self.p14 = 0.4  # frac of leaf loss transfered to lit (0.2 - 0.7)
        self.p15 = 1.2e-4  # labile turnover rate             (1e-4 - 0.1)
        self.p16 = 1.4e-1  # frac. of labile transfer respired   (1e-2 - 0.5)
        self.p17 = 130.5  # max c_fol value                   (100 - 500) g C m-2

        self.bnds = ((1e-6, 1e-2), (0.2, 0.7), (0.01, 0.5), (0.01, 0.5), (1e-4, 0.1),
                     (1e-6, 1e-2), (1e-4, 1e-2), (1e-5, 1e-2), (1e-6, 1e-2), (0.018, 0.2),
                     (10, 100), (200, 400), (8., 15.), (0.2, 0.7), (1e-4, 0.1), (1e-2, 0.5), (100, 500),
                     (10, 1000), (1e-4, 1000), (10, 1000), (100, 1e5), (10, 1000), (100, 2e5))

        self.bnds_tst = ((1e-6, 1e-2), (0.2, 0.7), (0.01, 0.5), (0.01, 0.5), (1e-4, 0.1),
                     (1e-6, 1e-2), (1e-4, 1e-2), (1e-5, 1e-2), (1e-6, 1e-2), (0.018, 0.2),
                     (10, 100), (200, 400), (8., 15.), (0.2, 0.7), (1e-4, 0.1), (1e-2, 0.5), (100, 500),
                     (10, 1000), (1e-4, 1000), (10, 1000), (100, 1e5), (10, 1000), (100, 2e5))
        ...
        self.param_dict = col.OrderedDict([('theta_min', self.p1),
                                          ...
                                          ('f_roo', self.p4), ('theta_fol', self.p5),
                                          ('theta_woo', self.p6), ('theta_roo', self.p7),
                                          ('theta_lit', self.p8), ('theta_som', self.p9),
                                          ('Theta', self.p10), ('ceff', self.p11),
                                          ('gdd_out', self.p12), ('t_fall', self.p13),
                                          ('f_lit', self.p14), ('theta_lab', self.p15),
                                          ('lab_resp', self.p16), ('max_cf', self.p17),
        ...
        self.dtype_dalec = [('theta_min', '<f8'),('f_auto', '<f8'), ('f_fol', '<f8'),
                            ('f_roo', '<f8'), ('theta_fol', '<f8'),
                            ('theta_woo', '<f8'), ('theta_roo', '<f8'),
                            ('theta_lit', '<f8'), ('theta_som', '<f8'),
                            ('Theta', '<f8'), ('ceff', '<f8'),
                            ('gdd_out', '<f8'), ('t_fall', '<f8'),
                            ('f_lit', '<f8'), ('theta_lab', '<f8'),
                            ('lab_resp', '<f8'), ('max_cf', '<f8'),
                            ('clab', '<f8'), ('cf', '<f8'),
                            ('cr', '<f8'), ('cw', '<f8'), ('cl', '<f8'),
                            ('cs', '<f8')]
        ...
        self.edinburgh_mean = np.array([(9.80983217e-04,   5.19025559e-01,   1.08612889e-01,
                                        4.84356048e-01,   1.2e-2,   1.01336503e-04,
                                        3.22465935e-03,   3.44239452e-03,   1.11320287e-04,
                                        4.14726183e-02,   7.14355778e+01,   240.,
                                        10.,   0.4,   1.2e-2,
                                        130.5,   1.28460812e+02,   1.36541509e+02,
                                        6.86396830e+01,   2.83782534e+02,   6.50600814e+03,
                                        5.98832031e+02,   1.93625350e+03)], dtype=self.dtype_dalec)

        self.reflex = np.array([(5e-06,   0.32,   0.45,
                                0.3,   0.08,   2.5e-4,
                                0.008,  0.04,   2e-4,
                                0.07,   7.14355778e+01,   250.,
                                9.5,   0.45,   0.08,
                                0.12, 130.5,  1.36541509e+02,
                                2.,   2.83782534e+02,   6.50600814e+03,
                                5.98832031e+02,   1.93625350e+03)], dtype=self.dtype_dalec)

        self.reflex2 = np.array([(5.00000000e-06,   3.20000000e-01,   4.50000000e-01,
                                  3.00000000e-01,   8.00000000e-02,   2.50000000e-04,
                                  8.00000000e-03,   4.00000000e-02,   2.00000000e-04,
                                  7.00000000e-02,   7.14355778e+01,   2.50000000e+02,
                                  9.50000000e+00,   4.50000000e-01,   8.00000000e-02,
                                  1.20000000e-01,   1.30500000e+02,   1.14598170e+02,
                                  1.46510111e-04,   1.05064099e+02,   7.00475644e+03,
                                  1.46436159e+01,   2.23149718e+03)], dtype=self.dtype_dalec)
        ...
        self.edinburgh_std = np.array([2.03001590e-03,   1.16829160e-01,   1.11585876e-01,
                                       ...
                                       1.62308654e-02,   2.04219069e+01,   100.,
                                       4.,   0.5,   1.16141739e-01,
                                       0.4,   100.,   6.62621885e+01,
        ...
        self.e_w_assim = np.array([(9.80983217e-04,   5.19025559e-01,   1.08612889e-01,
                                        4.84356048e-01,   1.2e-2,   1.01336503e-04,
                                        3.22465935e-03,   3.44239452e-03,   1.11320287e-04,
                                        4.14726183e-02,   240.,
                                        10.,   0.4,   1.2e-2,
                                        130.5,   1.28460812e+02,   1.36541509e+02,
                                        6.86396830e+01,   2.83782534e+02,   6.50600814e+03,
                                        5.98832031e+02,   1.93625350e+03)], dtype=self.dtype_east_west)
