...
class DalecData:
    ...
    def __init__(self, start_date=None, end_date=None, ob_str=None,
        ...
        self.p = np.array([6.43766778e-04,   4.27385622e-01,   3.22712690e-02,
                           3.70030025e-01,   1.10504749e+00,   9.86102255e-05,
                           5.13204711e-03,   4.72072225e-03,   1.27620660e-04,
                           9.57390688e-02,   6.53008240e+01,   1.58175477e+02,
                           6.55234665e-02,   1.89879919e+01,   3.06098578e+02,
                           5.30189262e+01,   2.93001470e+01,   6.22662734e+01,
                           1.26859002e+01,   2.32391924e+02,   7.39626567e+03,
                           1.51731357e+02,   1.95231902e+03])

        # I.C. for carbon pools gCm-2     range
        self.clab = 75.0               # (10,1e3)
        self.cf = 5.5                  # (10,1e3)
        self.cr = 135.0                # (10,1e3)
        self.cw = 13313.0              # (3e3,3e4)
        self.cl = 151.                  # (10,1e3)
        ...
        self.p12 = 158.  # d_onset, clab release date       (1 - 365) (60,150)
        self.p13 = 0.7  # f_lab, frac GPP to clab           (0.01 - 0.5)
        self.p14 = 20.  # cronset, clab release period      (10 - 100)
        self.p15 = 306.  # d_fall, date of leaf fall        (1 - 365) (242,332)
        self.p16 = 53.  # crfall, leaf fall period          (10 - 100)
        self.p17 = 29.2  # clma, leaf mass per area         (10 - 400) g C m-2
        ...
        self.xb_ew_lai = np.array([6.43766778e-04,   4.27385622e-01,   3.22712690e-02,
                                   3.70030025e-01,   1.10504749e+00,   9.86102255e-05,
                                   5.13204711e-03,   4.72072225e-03,   1.27620660e-04,
                                   9.57390688e-02,   6.53008240e+01,   1.58175477e+02,
                                   6.55234665e-02,   1.89879919e+01,   3.06098578e+02,
        ...
        self.xb_ew_lai_hi = np.array([5.47099980e-04,   4.49228870e-01,   4.09088649e-02,
                                      3.69550552e-01,   1.08939445e+00,   1.01196932e-04,
                                      5.41063421e-03,   4.38664659e-03,   1.31134514e-04,
                                      9.35399032e-02,   5.61843728e+01,   1.58422811e+02,
        ...
        for i in range(len(self.params4opt)):
            for j in range(len(self.params4opt)):
                self.opt_B[i, j] = self.B[self.params4opt[i], self.params4opt[j]]

        self.lower_bnd = np.array([1e-5, 0.3, 0.01,
                                   0.01, 1.0001, 2.5e-5,
                                   1e-4, 1e-4, 1e-7,
                                   0.018, 10., 60.,
                                   0.01, 10., 220.,
        ...
        self.upper_bnd = np.array([1e-2, 0.7, 0.5,
                                   0.5, 10., 1e-3,
                                   1e-2, 1e-2, 1e-3,
                                   0.1, 100., 185.,
                                   0.5, 100., 332.,
        ...
        self.sigo_cw = 23.  # from AH dbh measurements
        self.sigo_cr = 8.5  # from AH dbh measurements
        self.sigo_cl = 7.0  # 30%
        self.sigo_cs = 1500.0  # 30%
        self.sigo_nee = 1.  # 0.71  # g C m-2 day-1
        self.sigo_nee_day = 1.  # 0.71
        self.sigo_nee_night = 2*self.sigo_nee_day
        self.sigo_lf = 0.5
        self.sigo_lw = 0.5
        ...
        self.sigo_rtot = 0.71
        self.sigo_rh = 0.6
        self.sigo_lai = 0.07  # 0.5  # from AH optical measurements
        self.sigo_clma = 2.  # 5.0  # from AH litter scans
        self.sigo_donset = 5.
        self.sigo_dfall = 7.
        self.nee_day_scale = 0.025  # 0.03  # 0.05
        self.nee_night_scale = 3*0.025  # 0.03*3  # 0.15
    ...
    def assimilation_obs(self, ob_str, data, scale_nee=0):
        ...
        if scale_nee == 1:
            if 'nee' in obs_dict.keys():
                obs_err_dict['nee'] = abs(obs_dict['nee'])*self.nee_day_scale + 0.71 #0.71
            if 'nee_day' in obs_dict.keys():
                obs_err_dict['nee_day'] = abs(obs_dict['nee_day'])*self.nee_day_scale + 0.71 #0.71
            if 'nee_night' in obs_dict.keys():
                obs_err_dict['nee_night'] = abs(obs_dict['nee_night'])*self.nee_night_scale + 0.71*3 #1.2
